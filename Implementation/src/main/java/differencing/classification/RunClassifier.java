...
public class RunClassifier implements Classifier {
    ...
    public RunClassifier(
        ...
        if (isMissing) {
            ...
            // to run the differencing for the corresponding benchmark.
            this.classification = Classification.MISSING;
        } else if (isBaseToolMissing) {
            // A run should be classified as BASE_TOOL_MISSING if we don't have
            // any differencing results for the corresponding base tool (SE, DSE
            // or ARDiff). This can either be because the base tool was never
            // executed or because its execution ended with an error.
            ...
            if (neqCount > 0) {
                ...
                // partitions, classify the run as either:
                ...
                // ALL partitions in the current run are unreachable. Such a result
                // should not be possible, because it would imply that there can be
                // methods that have no possible program paths through them.
                // Therefore, the more likely explanation in such a case is that
                // our implementation is buggy in some way.
                assert unreachableCount == partitionCount;
                throw new RuntimeException("Run contains only unreachable partitions.");
            } else {
