...
{ //Maybe the solution should be changed, right now it looks concolic
    ...
    public SymbolicRealFunction (String cls, String mth, Class<?>[] ast,
        ...
        try {
            ...
            for (int i=0; i<args.length; i++)
                if (sym_args[i] instanceof IntegerExpression) {
                    if(argTypes[i].toString().equals("long"))
                        args[i] = new Long(((IntegerExpression)sym_args[i]).solution());
                    else{
                        int sol = (int)((IntegerExpression)sym_args[i]).solution();
                        if(argTypes[i].toString().equals("int"))
                            args[i] = new Integer(sol);
                        else{
                            boolean b = (sol == 1)?true:false;
                            args[i] = new Boolean(b);
                        }
                    }
                }
                else {// RealExpression
    ...
    public int compareTo(Expression expr) {
        if (expr instanceof SymbolicRealFunction) {
            SymbolicRealFunction e = (SymbolicRealFunction) expr;
